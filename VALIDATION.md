...
Version 0.5.2 writes routing diagnostics directly to the process stderr, independent of Cordis logger exporters. Child-process tests with real Harness components and no logger exporter verify actual scoring lines, exact scores, enabled tools, and silence when successful diagnostics are disabled. 68 tests pass. No paid API requests were needed.

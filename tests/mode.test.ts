...
for (const protocol of ['systemone', 'vercel', 'openai'] as const) test(`only Just enough tools mode starts empty and routes through the configured ${protocol} endpoint`, async t => {
  let scoringCalls = 0;
  ...
  const server = createServer(async (req, res) => {
    ...
    scoringCalls++;
    assert.equal(req.url, protocol === 'vercel' ? '/v1/evaluation-model' : protocol === 'openai' ? '/v1/chat/completions' : '/v1/systemone');
  ...
  await h.ctx.plugin(Settings,{protocol,model:'test-scorer',apiKey:'mode-test-key',baseUrl:`http://127.0.0.1:${address.port}/v1`});
